...
try {
    ...
    if ((Get-FileHash $installedExe).Hash -ne (Get-FileHash '.\dist\DotaBuildHelper.exe').Hash) { throw 'Installed payload mismatch' }
    ...
    if (-not $result.ok -or -not $result.frozen) { throw 'Installed app smoke failed' }
    ...
    [ordered]@{ok=$true;install=$true;shortcut=$true;installed_app_smoke=$true;reinstall=$true;
               uninstall=$true;settings_preserved=$true;report=$report} | ConvertTo-Json |
